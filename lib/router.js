...
var publicPages = ['index', 'login', 'logout', 'signup', 'company', 'companyPeople', 'newCompany', 'newCompanyAll'];
...
Router.route('/profile/:_id/projects', function() {
	...
	this.render('profile', {data: Meteor.users.findOne(this.params._id)});
...
Router.route('/profile/:_id/companies', function() {
	...
	this.render('listCompanies', {
		...
		data: function() {
			try {
				var companies = Meteor.users.findOne(this.params._id).profile.companies;
				if(!(companies && companies.length >= 0)) companies = [];
			} catch(e) {
				var companies = [];
			}
